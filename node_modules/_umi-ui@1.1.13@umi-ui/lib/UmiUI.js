...
var _getClientScript = _interopRequireDefault(require("./getClientScript"));
...
class UmiUI {
  ...
  openProject(key, service, opts) {
    ...
    if (!this.developMode && service) {
      ...
      // Attach Service
      debug(`Attach service for ${key}`); // Use local service and detect version compatibility
      ...
      if (process.env.UI_CHECK_LOCAL !== 'none' && localBin) {
        ...
        if (!_semver().default.satisfies(version, process.env.BIGFISH_COMPAT ? '^2.21.0-0' : '^2.10.0-0')) {
          throw new _ActiveProjectError.default({
            title: process.env.BIGFISH_COMPAT ? `本地项目的 Bigfish 版本（${version}）过低，请升级到 @alipay/bigfish@2.21 或以上，<a target="_blank" href="https://yuque.antfin-inc.com/bigfish/doc/uzfwoc#ff1deb63">查看详情</a>。` : {
      ...
      try {
        ...
        service.init();
        debug(`Attach service for ${key} ${_chalk().default.green('SUCCESS')}`);
        this.servicesByKey[key] = service;
      } catch (e) {
        ...
        var _ref2 = _asyncToGenerator(function* (resolve, reject) {
          ...
          function () {
            var _ref4 = _asyncToGenerator(function* (req, res) {
              ...
              if (process.env.LOCAL_DEBUG) {
                try {
                  const _ref5 = yield (0, _got().default)(`http://localhost:8002${req.path}`),
                        body = _ref5.body;
          ...
          function send(action) {
            ...
            debugSocket(_chalk().default.green.bold('>>>>'), formatLogMessage(message));
          ...
          ss.on('connection', conn => {
            conns[conn.id] = conn;
            debugSocket(`🔗 ${_chalk().default.green('Connected to')}: ${conn.id}`);
            ...
            conn.on('close', () => {
              debugSocket(`😿 ${_chalk().default.red('Disconnected to')}: ${conn.id}`);
            ...
            conn.on('data', message => {
              try {
                ...
                debugSocket(_chalk().default.blue.bold('<<<<'), formatLogMessage(message));
          ...
          const port = process.env.UMI_PORT || (yield _portfinder().default.getPortPromise({
            port: 3000
          }));
