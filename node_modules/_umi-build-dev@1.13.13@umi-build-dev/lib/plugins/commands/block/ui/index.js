...
var _default = api => {
  ...
  (0, _server.default)(api);

  function getRouteComponents(routes) {
    return routes.reduce((memo, route) => {
      if (route.component && !route.component.startsWith('()')) {
        const component = api.winPath(require.resolve((0, _path().join)(api.cwd, route.component)));
  ...
  if (process.env.NODE_ENV === 'development' && !api.config.ssr && command === 'dev') {
    api.modifyAFWebpackOpts(memo => {
      generateRouteComponents();
      memo.extraBabelPlugins = [...(memo.extraBabelPlugins || []), [require.resolve('../sdk/flagBabelPlugin'), {
...
(() => {
  ...
  window.GUmiUIFlag = require('${api.relativeToTmp(require.resolve('../sdk/flagBabelPlugin/GUmiUIFlag'))}').default;
